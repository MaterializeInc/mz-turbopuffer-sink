...
class FakeClient:
    def __init__(self, errors=None):
        self.namespaces = FakeNamespaces(errors)
...
class TestRequestShape:
    def test_mixed_ops_become_one_request(self):
        ...
        assert client.namespaces.requests == [
            {
                "namespace": "ns",
    ...
    def test_empty_op_kinds_omitted(self):
        ...
        assert client.namespaces.requests == [{"namespace": "ns", "deletes": [3]}]
...
class TestChunking:
    def test_splits_by_max_rows(self):
        ...
        sizes = [len(r["upsert_rows"]) for r in client.namespaces.requests]
        assert sizes == [2, 2, 1]
        written = [row["id"] for r in client.namespaces.requests for row in r["upsert_rows"]]
...
class TestRetries:
    def test_retries_429_then_succeeds(self):
        ...
        assert len(client.namespaces.requests) == 3
        assert len(sleeps) == 2
    ...
    def test_connection_error_is_retryable(self):
        ...
        assert len(client.namespaces.requests) == 2
